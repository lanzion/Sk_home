...
export default {
  ...
  created() {
    ...
    if (this.$store.state[falg.toLowerCase()].moreHotProduct.length == 0) {
      this.$store.dispatch("getMorehotsale", { id: falg });
    }
    if (this.$store.state[falg.toLowerCase()].moreHotProductImg.length == 0) {
      this.$store.dispatch("moreHotProductImg", { id: falg });
    }
    var This = this;
    var timer4 = setInterval(function() {
      if (
        This.$store.state[falg.toLowerCase()].moreHotProduct.length != 0 &&
        This.$store.state[[falg.toLowerCase()]].moreHotProductImg.length != 0
      ) {
  ...
  computed: {
    moreProduct() {
      ...
      return this.$store.state[falg].moreHotProduct;
    },
    moreProductImg() {
      var falg = this.$route.params.id.toLowerCase();
      return this.$store.state[falg].moreHotProductImg;
    }
  },
  components: {
    bannerComponent
